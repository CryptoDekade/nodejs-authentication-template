...
app.post('/register', async (req, res, next) => {
  try {
    ...
    if (err.isJoi) return res.status(422).redirect('/register')
    next(err)
...
app.post('/login', async (req, res, next) => {
  try {
    ...
    const refreshToken = jwt.sign({ _id: validUser._id }, REFRESH_TOKEN_SECRET)
...
app.post('/token', async (req, res) => {
  ...
  if (refreshToken === null) return res.status(400).send('refreshToken is null')
